...
type CalibreBook struct {
	...
	Formats      map[string]string // format -> filename without extension
...
func GetBookFilePath(libraryPath string, book *CalibreBook, preferredFormats []string) (string, string) {
	...
	for _, format := range preferredFormats {
		filenameBase, ok := book.Formats[format]
		if !ok {
			continue
		}
		filePath := filepath.Join(libraryPath, book.RelativePath, filenameBase+"."+strings.ToLower(format))
		if fileExists(filePath) {
			return filePath, strings.ToLower(format)
		}
	}
	return "", ""
}
