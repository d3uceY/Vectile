...
type CodeBlock struct {
	...
	SymbolPath string // enclosing symbol chain, e.g. "OrderService"
...
// splitNodeTypes defines which node types trigger structural splitting per
// language.
...
// GetSupportedCodeExtensions returns all file extensions the code parser
// handles.
...
// IsCodeFile reports whether a path is a supported code file.
...
// ParseCodeFile parses a source file into structural, size-bounded blocks
// using the "cAST" split-then-merge strategy: a named definition that fits
// the budget becomes one block; an oversized definition is split recursively
// along its AST child boundaries; runs of small adjacent trivia are merged up
// to the budget; word-window splitting is the last resort for oversized
// leaves. maxWords is the per-block word budget; overlap is used by the
// last-resort window splitter.
...
// chunkCtx holds the invariants for one file's recursive chunking.
...
// chunkNodes converts a sequence of sibling AST nodes into size-bounded
// blocks. parentPath is the enclosing symbol chain (empty at file scope);
// topLevel marks file-scope trivia as "module_top" (vs "block" when nested).
	...
	for _, node := range nodes {
		...
		case c.splits[node.Type()]:
			...
			if w <= c.maxWords {
				...
				// Oversized definition: recurse into children so the split
		...
		case w > c.maxWords:
			// Oversized trivia: recurse into children, or window-split a leaf.
			flush()
...
// spanBlock builds a block from a contiguous run of nodes, using the exact
// source span so original formatting is preserved. ok=false if the span is
// blank.
...
func plainTextDoc(sourceBytes []byte, language, relativePath string, maxWords, overlap int) *CodeDocument {
	...
	for _, w := range splitWords(fullText, maxWords, overlap) {
		if strings.TrimSpace(w) == "" {
			continue
		}
				...
				for j := 0; j < int(child.ChildCount()); j++ {
					...
					if gc.Type() == "variable_declarator" {
						for k := 0; k < int(gc.ChildCount()); k++ {
							if ggc := gc.Child(k); ggc.Type() == "identifier" {
		...
		if strings.Contains(child.Type(), "declarator") {
			for j := 0; j < int(child.ChildCount()); j++ {
				...
				if strings.Contains(gc.Type(), "declarator") {
					for k := 0; k < int(gc.ChildCount()); k++ {
						if ggc := gc.Child(k); ggc.Type() == "identifier" || ggc.Type() == "field_identifier" {
							return ggc.Content(sourceBytes)
						}
					}
				}
