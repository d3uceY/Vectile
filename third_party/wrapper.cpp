...
int llama_wrapper_embeddings(void* ctx, const char* text, float* embeddings, int max_embeddings) {
    ...
    try {
        ...
        // Evaluate tokens in chunks that respect n_batch limit. Each chunk is
        // an independent forward pass: positions restart at 0 and the KV
        // cache is cleared between chunks, so a text longer than the model's
        // sequence window can't overflow the position tables (which abort in
        // ggml get_rows) or collide cached positions. The pooled embedding of
        // the final chunk is what's returned.
        int n_batch = llama_n_batch(wrapper->ctx);
        ...
        for (int i = 0; i < n_tokens; i += n_batch) {
            ...
            // Clear KV so this chunk starts from a clean slate
            llama_memory_seq_rm(llama_get_memory(wrapper->ctx), 0, -1, -1);

            llama_batch batch = llama_batch_init(chunk_size, 0, 1);
            common_batch_clear(batch);

            // Add tokens for this chunk (positions relative to the chunk)
...
int llama_wrapper_embeddings_batch(void* ctx, const char** texts, int n_texts, float* embeddings, int n_embd) {
    ...
    try {
        ...
        // Flush the current batch: decode it, extract one embedding per
        // sequence, then reset the batch + KV state. Empty batch = no-op.
        ...
        // Embed a single text on its own, decoding in chunks of n_batch.
        // This mirrors llama_wrapper_embeddings and guarantees a text longer
        // than the batch capacity can never overflow the batch (which aborts
        // in common_batch_add with "llama_batch size exceeded") or overflow
        // the model's position tables. Each chunk is an independent forward
        // pass: positions restart at 0 and the KV cache is cleared between
        // chunks. The pooled embedding of the final chunk is returned.
        auto embed_single = [&](const std::vector<llama_token>& tokens) -> bool {
            ...
            for (int i = 0; i < n_tokens; i += n_batch) {
                ...
                llama_batch b = llama_batch_init(chunk_size, 0, 1);
                common_batch_clear(b);
                for (int j = 0; j < chunk_size; j++) {
                    // All tokens need logits for embeddings
                    common_batch_add(b, tokens[i + j], j, { 0 }, true);
                }
                if (llama_decode(wrapper->ctx, b) != 0) {
                    llama_batch_free(b);
                    g_last_error = "Failed to decode tokens for embeddings";
                    return false;
                }
        ...
        for (int k = 0; k < n_texts; k++) {
            ...
            // A single text larger than the batch capacity can't share a
            // batch (adding it would overflow llama_batch). Flush anything
            // pending, then embed it on its own via the chunked path.
            if (n_tokens > n_batch) {
                if (!flush_batch() || !embed_single(tokens)) {
                    llama_batch_free(batch);
                    return -1;
                }
                continue;
